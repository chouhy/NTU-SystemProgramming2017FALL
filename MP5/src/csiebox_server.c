...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
  ...
  *server = tmp;
  ...
  if(tmp->d){
    ...
    char ppp[PATH_MAX]="";
    sprintf(ppp,"%s/csiebox_server.pid",tmp->arg.run_path);
    FILE* fdd = fopen(ppp,"w");
    fprintf(fdd,"%d\n",getpid() );
    //write(fdd,&pidnum,4);
    fclose(fdd);
  ...
  sprintf(fifopath,"%s/fifo.%d",tmp->arg.run_path,getpid());
  mkfifo(fifopath,0644);
  
}
void sig_handle(int sig){
  if(sig == SIGUSR1){
    int ffd = open(fifopath,O_WRONLY);
